...
def main(args: DictConfig) -> None:
    ...
    # Use incorrect samples for implicit ambiguity detection stage (limit to 1000 samples)
    incorrect_samples_full = [sample for sample in explicit_results if sample.get('is_correct') is False]
    incorrect_samples = incorrect_samples_full[:1000]  # Limit to 1000 samples
    
    logger.info(f'Done explicit prediction. Correct {len(correct_samples)} samples, incorrect {len(incorrect_samples)}/{len(incorrect_samples_full)} samples (limited for implicit detection).')

    analysis(dataset=explicit_results, logger=logger)
    
    print("CURRENTLY HERE - 1")

    # clean GPU 0 memory here
