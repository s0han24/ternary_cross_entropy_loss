...
def main(args: DictConfig) -> None:
    ...
    ### HYPERPARAMETERS ####
    STAGE_INDEX = args.pipeline.stage_index
    ...
    except (AssertionError, FileNotFoundError, NotImplementedError) as e:
        logger.warning(f'AmbigTriviaQA dataset not found, skipping: {e}')
        ambig_triviaqa_test_dataset = []
    ...
    try:
        ...
        logger.warning(f'SituatedQA(Geo) dataset not found, skipping: {e}')
        situated_qa_geo_test_dataset = []
    ...
    try:
        ...
        logger.warning(f'SituatedQA(Temp) dataset not found, skipping: {e}')
        situated_qa_temp_test_dataset = []
    ...
    try:
        ...
        logger.warning(f'AmbigWebQuestions dataset not found, skipping: {e}')
        ambig_web_questions_test_dataset = []
    ...
    try:
        ...
        logger.warning(f'AmbigFreebaseQA dataset not found, skipping: {e}')
        ambig_freebase_qa_test_dataset = []
    ...
    if STAGE_INDEX == 1 and not os.path.isfile(ambig_triviaqa_evaluation_results_file):
        ...
        if len(ambig_triviaqa_test_dataset) > 0:
            ...
            logger.info('Skipping AmbigTriviaQA evaluation - dataset is empty.')
    else:
        logger.info(f'Initial evalution results already exists @ stage {STAGE_INDEX}.')

    # score, is_correct, prediction
    if len(ambig_triviaqa_test_dataset) > 0 and os.path.isfile(ambig_triviaqa_evaluation_results_file):
        ...
        logger.info('Skipping AmbigTriviaQA analysis - dataset is empty or results file not found.')
    ...
    if STAGE_INDEX == 1 and not os.path.isfile(situated_qa_geo_evaluation_results_file):
        ...
        if len(situated_qa_geo_test_dataset) > 0:
            if model is None:
                model, _ = load_vllm_model_and_tokenizer(args, stage_index=STAGE_INDEX, previous_model_checkpoint_path=previous_model_checkpoint_path)
            ...
            logger.info('Skipping SituatedQA Geo evaluation - dataset is empty.')
    else:
        logger.info(f'Initial evalution results already exists @ stage {STAGE_INDEX}.')

    # score, is_correct, prediction
    if len(situated_qa_geo_test_dataset) > 0 and os.path.isfile(situated_qa_geo_evaluation_results_file):
        ...
        logger.info('Skipping SituatedQA Geo analysis - dataset is empty or results file not found.')\
    ...
    if STAGE_INDEX == 1 and not os.path.isfile(situated_qa_temp_evaluation_results_file):
        ...
        if len(situated_qa_temp_test_dataset) > 0:
            if model is None:
                model, _ = load_vllm_model_and_tokenizer(args, stage_index=STAGE_INDEX, previous_model_checkpoint_path=previous_model_checkpoint_path)
            ...
            logger.info('Skipping SituatedQA Temp evaluation - dataset is empty.')
    else:
        logger.info(f'Initial evalution results already exists @ stage {STAGE_INDEX}.')

    # score, is_correct, prediction
    if len(situated_qa_temp_test_dataset) > 0 and os.path.isfile(situated_qa_temp_evaluation_results_file):
        ...
        logger.info('Skipping SituatedQA Temp analysis - dataset is empty or results file not found.')
    ...
    if STAGE_INDEX == 1 and not os.path.isfile(ambig_web_questions_evaluation_results_file):
        ...
        if len(ambig_web_questions_test_dataset) > 0:
            if model is None:
                model, _ = load_vllm_model_and_tokenizer(args, stage_index=STAGE_INDEX, previous_model_checkpoint_path=previous_model_checkpoint_path)
            ...
            logger.info('Skipping AmbigWebQuestions evaluation - dataset is empty.')
    else:
        logger.info(f'Initial evalution results already exists @ stage {STAGE_INDEX}.')

    # score, is_correct, prediction
    if len(ambig_web_questions_test_dataset) > 0 and os.path.isfile(ambig_web_questions_evaluation_results_file):
        ...
        logger.info('Skipping AmbigWebQuestions analysis - dataset is empty or results file not found.')
    ...
    if STAGE_INDEX == 1 and not os.path.isfile(ambig_freebase_qa_evaluation_results_file):
        ...
        if len(ambig_freebase_qa_test_dataset) > 0:
            if model is None:
                model, _ = load_vllm_model_and_tokenizer(args, stage_index=STAGE_INDEX, previous_model_checkpoint_path=previous_model_checkpoint_path)
            ...
            logger.info('Skipping AmbigFreebaseQA evaluation - dataset is empty.')
    else:
        logger.info(f'Initial evalution results already exists @ stage {STAGE_INDEX}.')

    # score, is_correct, prediction
    if len(ambig_freebase_qa_test_dataset) > 0 and os.path.isfile(ambig_freebase_qa_evaluation_results_file):
        ...
        logger.info('Skipping AmbigFreebaseQA analysis - dataset is empty or results file not found.')
